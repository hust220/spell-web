...
int main(int argc, char*argv[]){
  ...
  fileName = fileName.substr(0,fsize-4); 

  vector<string> vname = parse_line(fileName,'/'); 

  stringstream seq; 
  seq << ">" << vname.back() << "\n"; 

  vector< vector<advcoord> > vchain = getDataResidues(argv[1]);
  for(int i=0; i<vchain.size(); i++){
    vector<advcoord> vres = vchain[i]; 
    string resName = vres[0].resName;
    string letter; 
    if(resName.size()==3){
      letter = convert2letter(resName);
    } else if(resName.size()==1){
      letter = resName; 
    } else {
      cout << "wrong amino acid code " << endl;
      exit(0);
    }
    seq << letter; 
    if((i+1)%80==0) seq << "\n";
