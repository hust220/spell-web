...
int main(int argc, char*argv[]){
  ...
  */
  ...
  while (getline(in,line)){
    ...
    if(vfields[0]=="ASG"){
      nLines++;
      if(vfields[6]=="Coil" || vfields[6]=="Turn" || vfields[6]=="Bridge" || vfields[6]=="310Helix"){
  ...
  for(int i=0; i<vLoops.size(); i++){
    ...
    //    if(vTight[i]==1){
      ...
      for(int j=0; j<v1.size()-1; j++){
        int iflag = 0;
        if(menergy[v1[j].iRes]/minE<0.9) iflag++;
	...
	if((v1[j].cons<2 || v1[j].cons==100) && (v1[j+1].cons<2 || v1[j+1].cons==100)) iflag++;
        ...
        if(iflag==4) vsplit_temp.push_back(asite(v1[j],v1[j+1]));
