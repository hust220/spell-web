...
void print_help(){
  cout << "\nRenumbers medusa pdb file back to the numbering scheme of the original pdb file" << endl;
  cout << "Medusa pdb file will be replaced" << endl; 
  cout << "\nWRONG NUMBER OF PARAMETERS: \n" << endl; 
  cout << "./renumberBack pdb_file_with_original_numbering pdb_file_renumbered_by_medusa\n" << endl; 
  exit(0);
}
...
int main(int argc, char*argv[]){
  ...
  /*
  vector< vector<advcoord> > vchain = getData(argv[1]); 


  for(int i=0; i<vchain.size(); i++){
    cout << (vchain[i])[0].resSeq << " " << (vchain[i])[0].resName << endl;
  }
  */
  ...
  vector< vector<advcoord> > vchain_original = getDataResidues(argv[1]);

  //get medusa pdb file 
  vector< vector<advcoord> > vchain_medusa = getDataResidues(argv[2]);

  if(vchain_medusa.size() != vchain_original.size()){
    return EXIT_FAILURE;
  } 

  vector<advcoord> vresult;
  for(int i=0; i<vchain_original.size(); i++){
    vector<advcoord> vResOriginal = vchain_original[i]; 
    int iRes = vResOriginal[0].resSeq;    
    vector<advcoord> vResMedusa = vchain_medusa[i]; 
    for(int j=0; j<vResMedusa.size(); j++){
      vResMedusa[j].resSeq = iRes; 
      vresult.push_back(vResMedusa[j]);
    }
  }
  vresult.push_back(advcoord("-1"));
  write_pdb_as_orig(argv[2],vresult); 
