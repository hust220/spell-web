...
int main(int argc, char*argv[]){
  ...
  */
  ...
  int nChains = 0; 
  for(int i=0; i<vchain.size(); i++){
    if(vchain[i].name=="-1") nChains++;
    if(nChains==0) chainID = vchain[i].chainName;
  }
  filebuf outc;
  outc.open("chains.report",ios::out);
  ostream osc(&outc);
  stringstream schains;
  if(nChains>1){ 
    schains << chainID; 
    //    cout << chainID << endl;
  }
  ...
  cout << v1ch[0].resSeq <<  " " << v1ch[v1ch.size()-2].resSeq << endl; 
  write_pdb_as_orig("whole.pdb",v1ch); 


  filebuf outr;
  outr.open("cont.report",ios::out);
  ostream osr(&outr);
  stringstream scont; 
  int iResPrev = v1ch[0].resSeq; 
  for(int i=1; i<v1ch.size()-1; i++){
    if(v1ch[i].resSeq-iResPrev>1){
      scont << v1ch[i-1].chainName << "." << v1ch[i-1].resName << "." << v1ch[i-1].resSeq << " - " <<  
       v1ch[i].chainName << "." << v1ch[i].resName << "." << v1ch[i].resSeq << "\n"; 
    }
    iResPrev = v1ch[i].resSeq;
  }
  osr << scont.str(); 
  outr.close(); 
  ...
  */
  ...
  for(int i=0; i<v1ch.size(); i++){
    if(v1ch[i].name=="-1"){
      cout << "END: " << v1ch[i].name << endl; 
    } else {
      cout << v1ch[i].resName << " " << v1ch[i].resSeq << " " << v1ch[i].name << endl;
    }
  }
  */
}
